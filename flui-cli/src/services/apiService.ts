...
export class ApiService {
  ...
  async fetchModels(): Promise<Model[]> {
    try {
      ...
      // If no models have context_length, use popular models with Mistral as most powerful
      if (selectedModels.length === 0) {
        const popularModels = ['mistral-large', 'deepseek-r1-0528', 'gemini'];
        selectedModels = allModels
          .filter((m: any) => popularModels.includes(m.id))
          .map((m: any) => ({
            ...m,
            context_length: m.context_length || (m.id === 'mistral-large' ? 256000 : 128000),
            description: m.description || (m.id === 'mistral-large' ? 'Mistral Large - Most Powerful' : m.id)
          }))
          .sort((a: any, b: any) => b.context_length - a.context_length);
      }
